$.widget("nos.appwizard", {
    ...
    _create: function() {
        ...
        this.refreshCompileTab();

        setTimeout(function() {
            self.element.find('.add_model').click(); /* @todo: find a better solution */
        }, 250);
    ...
    initializeTabs: function() {
        var self =this;
        self.instances.tabs = this.element.find('.tabs');
        self.instances.tabs.css('display', 'block').nosOnShow();
        self.instances.tabs.wijtabs({
            alignment: 'left',
            show: function(e, ui) {
                ...
                if ($addField.length > 0 && $panel.find('.field_item').length == 0) {
                    $addField.click();
                    $panel.find('.is_on_appdesk').prop('checked', true);
                }
            }
        });
        ...
        self.instances.tabsMenu = this.element.find('.tabs > ul');
    },
    ...
    initializeApplicationSettings: function() {
        var $generalApplicationSettings = this.element.find('#general_application_settings');

        $generalApplicationSettings.find('.next_step').click(function(e) {
            e.preventDefault();
            self.instances.tabs.wijtabs('select', 1);
        });

        var self = this;
        this.instances.applicationSettingsName = $generalApplicationSettings.find('.application_settings_name');
        this.instances.applicationSettingsFolder = $generalApplicationSettings.find('.application_settings_folder');
        this.instances.applicationSettingsNamespace = $generalApplicationSettings.find('.application_settings_namespace');

        this.instances.applicationSettingsName.keyup(function() {
            var applicationName = self.instances.applicationSettingsName.val();
            self.instances.applicationSettingsFolder.val(self.toUnderscoreName(applicationName));
            self.instances.applicationSettingsNamespace.val(self.toSlashName(applicationName));
            self.refreshCompileTab();
        });

        this.instances.applicationSettingsFolder.keyup(function() {
            self.toUnderscoreNameInput(self.instances.applicationSettingsFolder);
            self.instances.applicationSettingsNamespace.val(self.toSlashName(self.instances.applicationSettingsFolder.val()));
            self.refreshCompileTab();
        });

        this.instances.applicationSettingsNamespace.keyup(function() {
            var namespace = self.instances.applicationSettingsNamespace.val();
            var newName = self.toSlashName(namespace);
            if (newName !== namespace) {
                self.instances.applicationSettingsNamespace.val(newName);
            }
            self.refreshCompileTab();
        });
    },
    ...
    addModel: function($el) {
        ...
        var $columnPrefix = $modelContent.find('.column_prefix');
        ...
        $modelName.keyup(function() {
            ...
            $tableName.val(self.toUnderscoreName(newName + 's'));
            $columnPrefix.val(self.toUnderscoreName(newName.substr(0, 4) + '_'));
            checkTabEnabled();
        });

        $tableName.keyup(function() {
            self.toUnderscoreNameInput($tableName);
            $columnPrefix.val(self.toUnderscoreName($tableName.val().substr(0, 4) + '_'));
            checkTabEnabled();
        });

        $columnPrefix.keyup(function() {
            self.toUnderscoreNameInput($columnPrefix);
            checkTabEnabled();
        });
        ...
        var fieldLayoutId = 'field_layout_' + self.modelId;

        $modelContent.find('.model_name').keyup(function() {
            self.refreshWizard($(this));
        });

        var $fieldLayout = $('<div class="field_layout"></div>', self.modelId);
        ...
        $fieldLayout.find('.categories_list').data('key', 'models[' + self.modelId + '][categories]');

        var ni = self.modelId;
        $fieldLayout.find('.next_step').click(function(e) {
            e.preventDefault();
            self.instances.tabs.wijtabs('select', ni * 2 + 2);
        });



        var fieldsId = 'fields_' + self.modelId;
        ...
        $fields.find('.model_fields').data('key', 'models[' + self.modelId + '][fields]');

        var ni = self.modelId;
        $fields.find('.next_step').click(function(e) {
            e.preventDefault();
            self.instances.tabs.wijtabs('select', ni * 2 + 3);
        });

        setEnabledTabs(false);
        function setEnabledTabs(enabled) {
            var action = enabled ? 'enableTab' : 'disableTab';
            self.instances.tabs.wijtabs(action, ni * 2 + 1);
            self.instances.tabs.wijtabs(action, ni * 2 + 2);

            self.refreshCompileTab();
        }

        self.refreshWizard($el);
    ...
    addField: function($el) {
        ...
        $fieldContent.find('.is_title_checkbox').change(function() {
            ...
            if ($this.is(':checked')) {
                ...
                $isOnAppdeskCheckbox.prop('checked', true);
                $isOnCrudCheckbox.prop('checked', true);
        ...
        var $columnInput = $fieldContent.find('.column_input');
        var $labelInput = $fieldContent.find('.label_input');

        $useTitleCheckbox.change(function () {
            refreshUseTitleCheckbox($(this));
        });
        ...
        $columnInput.keyup(function() {
            self.toUnderscoreNameInput($columnInput);
        });

        refreshUseTitleCheckbox($useTitleCheckbox);
        refreshIsOnCrudCheckbox($isOnCrudCheckbox);

        function titleToColumn() {
            if ($useTitleCheckbox.is(':checked')) {
                $columnInput.val(self.toUnderscoreName($labelInput.val()));
            }
        }

        function refreshUseTitleCheckbox($el) {
            $columnInput.attr('readonly', $el.is(':checked'));
            titleToColumn();
        }

        function refreshIsOnCrudCheckbox($el) {
            var $crudOptions = $el.closest('.field_item').find('.crud_options');
            $el.is(':checked') ? $crudOptions.removeClass('inactive') : $crudOptions.addClass('inactive');
        }

        self.refreshWizard($el);
        self.refreshCategories($el);
    },

    refreshCategories: function($el) {
    ...
    refreshCompileTab: function() {
        var self = this;
        var isApplicationSet = this.instances.applicationSettingsName.val() !== '' &&
            this.instances.applicationSettingsFolder.val() !== '' &&
            this.instances.applicationSettingsNamespace.val() !== '';
        var compileAction = (self.instances.tabs.find('.ui-state-disabled').length > 1 || !isApplicationSet) ? 'disableTab' : 'enableTab';
        var length = self.instances.tabs.wijtabs('length');
        self.instances.tabs.wijtabs(compileAction, length - 1);
    },


    refreshWizard: function($el) {
        var self = this;
        var $menuItems = self.instances.tabs.find('li a');
        var $modelNames = self.element.find('.model_name');
        $modelNames.each(function(i) {
            var $this = $(this);
            var $menuFieldsLayoutItem = $($menuItems[i * 2 + 1]);
            var $menuFieldsItem = $($menuItems[i * 2 + 2]);
            if ($modelNames.length > 1) {
                $menuFieldsLayoutItem.text(self.__('Step {{num}}. ({{modelName}}) Fields layout').replace('{{modelName}}', $this.val()).replace('{{num}}', i * 2 + 2));
                $menuFieldsItem.text(self.__('Step {{num}}. ({{modelName}}) Fields').replace('{{modelName}}', $this.val()).replace('{{num}}', i * 2 + 3));
            } else {
                $menuFieldsLayoutItem.text(self.__('Step {{num}}. Fields layout').replace('{{num}}', i * 2 + 2));
                $menuFieldsItem.text(self.__('Step {{num}}. Fields').replace('{{modelName}}', $this.val()).replace('{{num}}', i * 2 + 3));
            }
        });
        var $menuFieldsCompile = $($menuItems[$modelNames.length  * 2 + 1]);
        $menuFieldsCompile.text(self.__('Step {{num}}. Compile').replace('{{num}}', $modelNames.length * 2 + 2));
    },

    // Found on: http://stackoverflow.com/questions/990904/javascript-remove-accents-in-strings
    ...
    toUnderscoreNameInput: function($input) {
        var name = $input.val()
        var newName = this.toUnderscoreName(name);
        if (name !== newName) {
            $input.val(newName);
        }
    },
    ...
    toSlashName: function(str) {
        var str = this.removeDiacritics(str).replace(/([^A-Z|a-z| |\\|_])/g, '').trim().replace(/(_)/g, ' ');
        if (str.substr(0, 1) === '\\') {
            str = str.substr(1);
        }
        str = str.split(' ');
    ...
    processInputList: function($el, inputSelector) {
        var self = this;
        var prefix = '';
